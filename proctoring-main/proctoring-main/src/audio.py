...
# Global variable to control the sound stream
...
def sound():
    with sd.Stream(callback=print_sound):
        while not stop_sound_event.is_set():
            sd.sleep(100)  # Sleep for a short duration to avoid busy waiting


def stop_sound():
    stop_sound_event.set()  # Signal to stop the sound stream
